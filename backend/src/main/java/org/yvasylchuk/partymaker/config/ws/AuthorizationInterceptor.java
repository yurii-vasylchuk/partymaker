...
public class AuthorizationInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        final PartymakerPrincipal user = authorizationHeader != null && authorizationHeader.startsWith(BEARER_PREFIX) ?
        ...
        boolean shouldBlock = INTERCEPTING_PATHS.stream().anyMatch(path -> {
            ...
            return groups.contains(USER_ID_GROUP) && !matcher.group(USER_ID_GROUP).equals("%s".formatted(user.id()));
        });
